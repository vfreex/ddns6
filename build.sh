...
export CC=clang
export CFLAGS="--target=$TARGET -fPIC -march=mips32r2 -mtune=24kc -msoft-float --sysroot=$SYSROOT"
...
export RUSTFLAGS="-C target-feature=-crt-static -C linker-flavor=ld.lld -C linker=ld.lld $OBJS -C link-arg=--no-pie -C link-arg=-dynamic-linker=/lib/ld-musl-mips-sf.so.1 -L native=$SYSROOT/lib -L native=$LIBGCC_STATIC -L native=$LIBGCC_SHARED -C link-arg=-lgcc"
